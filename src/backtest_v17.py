...
def main():
    ...
    # --------------------------------------------------
    # V17: RANK-BASED SIGNAL STRATEGY
    #
    # Instead of choosing an absolute prediction threshold,
    # trade only the strongest predictions within each
    ...
    def run_rank_strategy(dataset, percentile):

        dataset = dataset.reset_index(drop=True)

        position = np.zeros(len(dataset))

        # Prediction cutoff based only on this dataset.
        cutoff = dataset["prediction"].quantile(
            1.0 - percentile
        )
    ...
    final_result = run_rank_strategy(
        final_test,
        BEST_PERCENTILE
    )
